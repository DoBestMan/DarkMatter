...
export const create = (address, provider) => {
  ...
  // READ
  const balanceOf = (owner, id) => contract.balanceOf(owner, id).then(parseInt);

  const tokenMaxSupply = (id) => contract.tokenMaxSupply(id).then(parseInt);

  const totalSupply = (id) => contract.totalSupply(id).then(parseInt);

  const uri = (id) => contract.uri(id);
